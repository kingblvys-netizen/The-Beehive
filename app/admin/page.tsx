...
export default function AdminDashboard() {
  ...
  const handleDecision = async (id: number, decision: string) => {
    if (decision === 'purge' && !confirm("Permanently delete this record?")) return;
    setProcessingId(id);
    try {
  ...
  if (status === "loading" || loading) return <div className="min-h-screen bg-black flex items-center justify-center font-mono text-yellow-500 animate-pulse">Establishing Secure Uplink...</div>;
    ...
    <div className="min-h-screen bg-[#050505] text-white font-mono selection:bg-yellow-500 selection:text-black relative cursor-none antialiased">
      ...
      <nav className="border-b border-white/5 bg-black/80 backdrop-blur-xl sticky top-0 z-40 p-6 flex justify-between items-center">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => router.push('/')}>
          <Hexagon className="text-yellow-500 fill-yellow-500" size={20} />
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        {/* Recruitment Toggles */}
        <div className="mb-12">
          <h3 className="text-[10px] font-black uppercase text-neutral-600 mb-6 flex items-center gap-2"><Power size={12} /> Active Channels</h3>
        ...
        <div className="bg-neutral-900/30 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-md shadow-2xl">
          <div className="p-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 bg-white/[0.01]">
            <h2 className="text-[11px] font-black uppercase tracking-[0.4em] text-neutral-400">Archived Intel</h2>
            <div className="relative w-full md:w-80">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-600" size={14} />
              <input type="text" placeholder="FILTER BY IDENTITY..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-black/50 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-[10px] focus:border-yellow-500 outline-none transition-all placeholder:text-neutral-700 font-bold uppercase" />
            </div>
          </div>
          <table className="w-full text-left">
            <thead className="text-[9px] font-black uppercase text-neutral-500 tracking-widest bg-white/[0.01]">
              <tr><th className="p-6">Identity</th><th className="p-6">Objective</th><th className="p-6">Status</th><th className="p-6 text-right">Action</th></tr>
            </thead>
            <tbody className="divide-y divide-white/[0.02]">
              {filteredApps.map((app) => (
                <tr key={app.id} className="group hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="p-6"><div className={`text-[10px] font-black uppercase flex items-center gap-2 ${app.status === 'approved' ? 'text-green-500' : app.status === 'declined' ? 'text-red-500' : 'text-yellow-500'}`}><span className={`w-1.5 h-1.5 rounded-full ${app.status === 'approved' ? 'bg-green-500' : app.status === 'declined' ? 'bg-red-500' : 'bg-yellow-500 animate-pulse'}`} />{app.status}</div></td>
                  <td className="p-6 text-right"><button onClick={() => setSelectedApp(app)} className="px-5 py-2.5 border border-white/10 hover:border-yellow-500 text-[10px] font-black uppercase rounded-lg transition-all">Review Dossier</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* --- SCROLLABLE SIDEBAR DOSSIER (FIXES QUESTIONS) --- */}
      <AnimatePresence>
        {selectedApp && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/98 backdrop-blur-3xl z-50 flex overflow-y-auto">
              ...
              <div className="md:w-1/3 p-8 md:p-12 border-b md:border-b-0 md:border-r border-white/5 md:sticky md:top-0 h-fit md:h-screen flex flex-col">
                ...
                <div className="mt-auto space-y-3">
                   <div className="grid grid-cols-1 gap-3">
                      <button disabled={processingId === selectedApp.id} onClick={() => handleDecision(selectedApp.id, 'approved')} className="w-full py-4 bg-green-500/10 text-green-500 border border-green-500/20 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-green-500 hover:text-black transition-all">Authorize</button>
                      <button disabled={processingId === selectedApp.id} onClick={() => handleDecision(selectedApp.id, 'declined')} className="w-full py-4 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-red-500 hover:text-white transition-all">Terminate</button>
                      {selectedApp.status !== 'pending' && (
                        <button disabled={processingId === selectedApp.id} onClick={() => handleDecision(selectedApp.id, 'reset')} className="w-full py-3 bg-yellow-500/5 text-yellow-500/50 border border-yellow-500/10 rounded-xl font-black uppercase text-[9px] tracking-widest hover:bg-yellow-500/10 hover:text-yellow-500 transition-all">Reset Decision</button>
                      )}
                      <button disabled={processingId === selectedApp.id} onClick={() => handleDecision(selectedApp.id, 'purge')} className="w-full py-3 bg-red-900/5 text-red-900/50 border border-red-900/10 rounded-xl font-black uppercase text-[9px] tracking-widest hover:bg-red-600 hover:text-white transition-all group"><Trash2 size={14} className="group-hover:animate-pulse" /> Purge Record</button>
                   </div>
                </div>
              </div>

              {/* MAIN CONTENT: THE QUESTIONS & ANSWERS */}
              <div className="md:w-2/3 p-8 md:p-12 md:py-24 bg-[#080808]">
                ...
                <div className="space-y-12">
                  {selectedApp.answers && Object.entries(selectedApp.answers).map(([key, value]) => {
                     // IMPROVED LOOKUP: Normalize matching to find role in staticRoleData
                     const roleEntry = staticRoleData.find(r => 
                        r.title.toLowerCase().trim() === selectedApp.role_title.toLowerCase().trim()
                     );
                     
                     // Fetch all questions for this specific role
                     const allQuestionsForThisRole = roleEntry ? getQuestions(roleEntry.id) : [];
                     
                     // Find the specific question label (e.g., "What is your age?") matching the answer key
                     const questionLabel = allQuestionsForThisRole.find(q => q.id === key)?.label || key.replace(/_/g, ' ');
