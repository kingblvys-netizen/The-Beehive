...
const SITE_VERSION = "2.6.0-TACTICAL";
      ...
      try {
        ...
        if (status === "authenticated") {
          ...
          if (Array.isArray(userApps)) {
            const submittedIds = userApps.map((app: any) => {
              const roleMatch = staticRoles.find(r => r.title === app.role_title);
              return roleMatch ? roleMatch.id : null;
            }).filter(id => id !== null);
            setSubmittedRoles(submittedIds);
          }
        }
      } catch (err) { console.warn("Satellite Sync Offline"); }
    };
    ...
    <div className="min-h-screen bg-[#050505] text-white relative overflow-hidden selection:bg-yellow-500 selection:text-black cursor-none font-sans antialiased">
               ...
               <div key={i} className="bg-neutral-900/40 border border-white/5 p-8 rounded-3xl backdrop-blur-md hover:border-white/10 transition-all group shadow-inner">
                 ...
                 <div>
                    <div className="text-2xl font-black text-white flex items-center gap-3 tracking-tighter">
                       {stat.pulse && <span className="relative flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"/><span className="relative inline-flex rounded-full h-3 w-3 bg-green-500"/></span>}
                       {stat.val}
                    </div>
                ...
                <motion.div key={role.id} variants={itemVariants} 
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-12">
                      <div className={`p-5 rounded-2xl transition-all duration-700 ${isPending ? 'bg-yellow-400/10 text-yellow-400' : 'bg-neutral-900 border border-white/5 group-hover:bg-yellow-400 group-hover:text-black group-hover:shadow-[0_0_25px_#FACC1588]'}`}>
                          <IconComponent size={28} />
                      </div>
                      {!role.isOpen ? (
                        <span className="text-[9px] px-3 py-1.5 rounded-full border border-red-500/20 text-red-500 bg-red-500/5 font-black uppercase flex items-center gap-1"><Lock size={10} /> Locked</span>
                      ) : isPending ? (
                        <span className="text-[9px] px-3 py-1.5 rounded-full border border-yellow-400/20 text-yellow-400 bg-yellow-400/5 font-black uppercase flex items-center gap-1"><Clock size={10} /> In Sync</span>
                      ) : (
                        <span className={`text-[9px] px-3 py-1.5 rounded-full border font-black uppercase ${role.level === 'HIGH' ? 'border-red-500/30 text-red-500 bg-red-500/5' : 'border-green-500/30 text-green-500 bg-green-500/5'}`}>{role.level} PRIORITY</span>
                      )}
                    </div>
                    <h3 className="text-3xl font-black mb-3 text-white group-hover:text-yellow-400 transition-colors uppercase italic tracking-tighter">{role.title}</h3>
                    <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-600 mb-8 flex items-center gap-2"><Database size={12} className="text-yellow-500/40" /> Commitment: {role.commitment}</div>
                    <p className="text-neutral-500 text-sm mb-12 leading-relaxed font-bold uppercase tracking-tight line-clamp-3 min-h-[60px] border-l-2 border-white/5 pl-6 group-hover:border-yellow-400/40 transition-colors">{role.description}</p>
                    {isPending ? (
                      <div className="w-full py-5 text-center border border-dashed border-yellow-400/20 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] text-yellow-400/60 italic bg-yellow-400/[0.02]">Linked Under Review</div>
                    ) : !role.isOpen ? (
                      <div className="w-full py-5 bg-white/5 border border-white/5 rounded-2xl text-center text-[10px] font-black uppercase tracking-[0.3em] text-neutral-700 italic">Terminal Offline</div>
                    ) : (
                      <Link href={`/apply/${role.id}`} className="block w-full">
                        <div className="group/btn relative w-full flex items-center justify-center py-5 bg-white/[0.04] border border-white/5 hover:bg-yellow-400 text-neutral-400 hover:text-black font-black text-[11px] uppercase tracking-[0.4em] rounded-2xl transition-all duration-300">Initiate Handshake <ChevronRight size={16} className="ml-1" /></div>
                      </Link>
                    )}
                  </div>
                </motion.div>
      ...
      <footer className="border-t border-white/5 bg-black/80 backdrop-blur-md py-16 relative z-10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="flex items-center gap-4 opacity-40 hover:opacity-100 transition-opacity group cursor-default">
          ...
          <div className="flex items-center gap-12">
            <Link href="/status" className="text-[10px] font-black uppercase text-neutral-600 hover:text-green-400 transition-all flex items-center gap-3 group">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /> Core Integrity
            </Link>
            <div className="flex items-center gap-8 border-l border-white/10 pl-12">
              <a href="https://discord.gg/qR6kFuBhCh" target="_blank" className="text-neutral-700 transition-all hover:scale-125 hover:text-[#5865F2]"><DiscordIcon size={24} /></a>
              <a href="https://www.twitch.tv/its_pupbee" target="_blank" className="text-neutral-700 transition-all hover:scale-125 hover:text-[#9146FF]"><TwitchIcon size={24} /></a>
              <a href="https://www.tiktok.com/@its_pupbee?lang=en" target="_blank" className="text-neutral-700 transition-all hover:scale-125 hover:text-[#FE2C55]"><TikTokIcon size={24} /></a>
            </div>
          </div>
        </div>
      </footer>
